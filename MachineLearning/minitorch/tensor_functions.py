...
def make_tensor_backend(tensor_ops, is_cuda=False):
    ...
    Args:
        tensor_ops (:class:`TensorOps`) : tensor operations object see `tensor_functions.py`
    ...
    class Backend:
        ...
        class Mean(Function):
            ...
            def forward(ctx, a, dim):
                # TODO: Implement for Task 2.2.
                ctx.save_for_backward(a.shape, dim, a.size)
                if dim is not None:
            ...
            def backward(ctx, grad_output):
                # TODO: Implement for Task 2.3.
                a_shape, dim, a_size = ctx.saved_values
                if dim is None:
                    out = grad_output.zeros(a_shape)
                    out._tensor._storage[:] = grad_output[0]
                    return out / a_size
