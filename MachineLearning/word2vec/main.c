...
void InitNet() {
    ...
    syn0 = malloc((long long)vocab_size * layer1_size * sizeof(real) * 128);
    if (syn0 == NULL) {printf("Memory allocation failed\n"); exit(1);}

    // 层次softmax的结构
    if (hs) {
        // 映射层到输出层之间的权重
        syn1 = malloc((long long)vocab_size * layer1_size * sizeof(real) * 128);
    ...
    // 负采样的结构
    if (negative>0) {
        syn1neg = malloc((long long)vocab_size * layer1_size * sizeof(real) * 128);
